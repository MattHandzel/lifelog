    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Err(e) = check {
            return Err(std::io::Error::new(
                ErrorKind::NotFound,
                format!("Failed to check for imagesnap: {}. Make sure it's installed.", e)
            ));
...
pub async fn start_logger(config: &CameraConfig) {
    ...
    match check_result {
        ...
        _ => {
            ...
            let devices_result = Command::new("imagesnap")
                .arg("-l")
                .output();
    ...
    println!("✓ Camera checks passed! Starting capture loop with interval: {} seconds", config.interval);
                ...
                if output.status.success() {
                    ...
                    match fs::metadata(&output_path) {
                        Ok(meta) if meta.len() > 0 => {
                            println!("  ✓ Saved image: {} ({} bytes)", output_path_str, meta.len());
                        },
                        Ok(_) => {
