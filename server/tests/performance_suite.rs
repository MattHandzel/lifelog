...
use lifelog_types::{BrowserFrame, QueryRequest, Query, GetDataRequest};
...
async fn test_performance_suite_smoke() {
    ...
    println!("Ingest Throughput: {:.2} records/sec (Total: {:?})", ingest_throughput, ingest_duration);
    ...
    println!("Query Latency: {:?} (Results: {})", query_duration, keys.len());
    ...
    println!("GetData Latency: {:?} (Results: {})", get_duration, results.len());

    // 5. Baseline Thresholds
    assert!(ingest_throughput > 10.0, "Ingest throughput too low: {:.2} < 10.0", ingest_throughput);
    assert!(query_duration < Duration::from_millis(1000), "Query latency too high: {:?}", query_duration);
    assert!(get_duration < Duration::from_millis(2000), "GetData latency too high: {:?}", get_duration);
}
