...
fn get_process_info(users_cache: &UsersCache) -> Result<Vec<ProcessInfo>, std::io::Error> {
    ...
    for entry in fs::read_dir("/proc")? {
        ...
        if let Ok(pid) = pid_str.parse::<i32>() {
            ...
            if let Ok(status) = fs::read_to_string(entry.path().join("status")) {
                for line in status.lines() {
                    ...
                    match parts.next() {
                        ...
                        Some("PPid:") => process.ppid = parts.next().unwrap_or("0").parse().unwrap_or(0),
                        Some("Uid:") => {
                            if let Some(uid) = parts.next().and_then(|s| s.parse().ok()) {
                                process.user = users_cache.get_user_by_uid(uid).map(|u| u.name().to_string_lossy().into_owned());
                            }
                        }
                        Some("Threads:") => process.threads = parts.next().unwrap_or("0").parse().unwrap_or(0),
                        Some("VmRSS:") => process.memory_usage = parts.next().and_then(|s| s.parse().ok()),
                        _ => {}
                    }
                }
            }
            ...
            // Read exe symlink
            process.exe = fs::read_link(entry.path().join("exe")).ok()
