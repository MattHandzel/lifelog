...
async fn get_screenshots(
    ...
    let mut stmt = match conn
        .prepare("SELECT rowid, timestamp FROM screen ORDER BY timestamp DESC LIMIT ? OFFSET ?")
    {
        Ok(stmt) => stmt,
    ...
    let screenshot_iter = match stmt.query_map(params![page_size, offset], |row| {
        ...
        if !full_path.exists() {
            println!(
                "WARNING: Generated path does not exist on disk: {:?}",
                full_path
            );
...
async fn update_screenshot_settings(
    ...
    panic!("This needs to be refactored");
    return Ok(());
    ...
    //// TODO: Send a command to the lifelog-logger to restart the screen logger
    ...
    //if (enabled && !old_config.enabled)
    //    || (enabled && old_config.enabled && interval != old_config.interval)
    //{
...
async fn get_screenshot_data(filename: String) -> Result<String, String> {
    ...
    if !file_path.exists() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let alt_path =
                screenshot_dir.join(Path::new(&clean_filename).file_name().unwrap_or_default());
            ...
            if alt_path.exists() {
                ...
                return match std::fs::read(&alt_path) {
                    ...
                    Err(e) => Err(format!(
                        "Failed to read screenshot from alternative path: {}",
                        e
                    )),
                };
            }
...
async fn trigger_camera_capture(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if !check.status.success() {
            return Err(
                "imagesnap utility not found. Please install with: brew install imagesnap"
                    .to_string(),
            );
        }
        ...
        if !devices_str.contains("Video Devices:") || devices_str.lines().count() <= 1 {
            return Err(
                "No cameras were detected. Check camera connections and permissions.".to_string(),
            );
        }
        ...
        match fs::metadata(&output_path) {
            ...
            Ok(_) => {
                return Err(
                    "Image was captured but file is empty. Camera may not be working properly."
                        .to_string(),
                );
            }
            Err(e) => {
...
async fn restart_camera_logger(
    ...
    println!(
        "Manually starting camera logger with interval: {} seconds",
...
async fn start_microphone_recording() -> Result<(), String> {
    ...
    update_microphone_settings(
        true,
        lifelog_interface_lib::config::load_config()
            .microphone
            .chunk_duration_secs,
        None,
    )
...
async fn get_audio_files(page: usize, page_size: usize) -> Result<Vec<AudioFile>, String> {
    ...
    let files = storage::get_audio_files(output_dir, page, page_size).map_err(|e| e.to_string())?;
