...
export default function TimelineDashboard({ collectorId = null }: TimelineDashboardProps): JSX.Element {
  ...
  async function handleSearch(): Promise<void> {
    ...
    try {
      ...
      if (entries.length > 0) {
        const keys = entries.slice(0, 20).map(e => ({ uuid: e.uuid, origin: e.origin }));
        try {
          ...
          if (matchCount < enriched.length) {
            setError(`${enriched.length}fr ${matchCount}match E0:${entries[0]?.uuid?.slice(0,12)} R0:${enriched[0]?.uuid?.slice(0,12)} R1:${enriched[1]?.uuid?.slice(0,12)}`);
          }
